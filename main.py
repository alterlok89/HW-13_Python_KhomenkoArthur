...
class stadium():
    ...
    def set_year(self, year):
        if str(year).isdigit():
            if 1800 < int(year) < 2022:
                self.__year = year
            else:
                print('Не верно указан год открытия!')
    ...
    def change_capacity(self, new_capacity):
        if new_capacity.isdigit():
            if int(new_capacity) > 0:
                ...
                print('Не верно указана вместимость стадиона! Вместимость не изменена!')
        else:
            print('Не верно указана вместимость стадиона! Вместимость не изменена!')

    def __str__(self):
        return (f'Название стадиона: {self.__name}\n'
                f'Год открытия: {self.__year}\n'
                f'Страна: {self.__country}\n'
                f'Город: {self.__city}\n'
                f'Вместимость: {self.capacity}')


obj_stadium = stadium('"Энфилд"', '28 сентября 1884', 'Англия', 'Ливерпуль', 53394)
print(obj_stadium)
